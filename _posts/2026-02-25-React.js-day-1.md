...
-> 기능(Action) 하나가 아닌 여러개가 동시에 움직이면? 갱신되는 Model 값이 추적이 불가하다.
...
앞부분에서 페이스북이 복잡해지면서 Flux 패턴이 등장한 이야기를 했었다. 이제부터는 그 흐름이 실제 React 코드와 엔진에서 어떻게 드러나는지, 최대한 설명해보고자 한다. 설명해보고자 하는 내용은 아래와 같다.
...
class Counter extends React.Component {
  constructor(props){
    ...
    // this.state : store의 상태.
    this.state = {count:0};
  }

  handleClick=()=>{
    // Action + Dispatcher 영역

    // handleClick(Action) : 이벤트 트리거를 받는다.
    // setState(Dispatcher) : store에 있는 state속성에 접근해서 상태변경.
...
```jsx
  class Counter extends React.Component{
    ...
    // lifeCycle : View나 Store가 변경 됐을 경우 후처리.
    componentDidMount(){
      // 라이프사이클 1
...
// 1. 'View'에서 사용자가 버튼을 클릭한다.
// 2. button onClick에서 setCount(count+1)을 해주는 'Dispatcher'가 실행된다.

// 3. 'Store'영역의 const [count, setCount] = useState(0)에서 setCout가 실행되면서, coun함수의 값, useState로 초기값으로 잡힌 0이 1로 갱신된다.

// 4. 'lifeCycle'영역에서 count함수값이 갱신되면서,
// useEffect(() => {
//    console.log('현재 count:',count);
// }, [count]);
// 에서 설정된 count 콜백 함수가 갱신되고 useEffect의 함수가 작동하면서 후처리 결과 console.log의 내용을 표시해준다.
...
상기의 세개의 부분은 중요하지만...[다음 글](https://hwp-log.github.io/react.js/2026/03/02/React.js-day-2.html)에서 차후 다뤄보기로 하겠다. 현재는 "학습커브가 왜 가파른지"에 대해 언급하는 내용이기에 문맥을 해칠수 있어서 건너뛰는 것이다. 분명한것은 Vanilla JS인 경우 수학적으로 딱딱 떨어지는 분자단위의 논리와 같다면, react와 같은 프레임워크는 프로토콜같은 추상단위의 개념으로 봐야 이해하기 용이하다는 것이다. 간단히 얘기하자면, 논리의 앞뒤가 맞기 이전에 의도가 무엇인지 부터 파악해야 한다는 것이다. 그 뒤에 안의 논리를 분자단위로 쪼개도 이해는 늦지 않다.

### 5. React 엔진 - "처음부터 끝까지 그리자"에서 "단위별로 쪼개서 그리자"
...
### 6. 동시성 랜더링 - "급한 것 먼저, 그렇지 않은것은 나중에"
Fiber가 "작업을 쪼갤 수 있는 구조"라면,
동시성(concurrent)랜더링은 그 위에서 돌아가는 "우선순위와 스케줄링 정책"이라고 보면 된다.

예시로 검색 필터 UI를 생각해 보자.

작동은 두가지로 구성된다.
1. 입력창에 실시간 입력 (자 -> 자동 -> 자동차)
2. 리스트가 실시간 필터링됨(자 : [자전거, 자동문, 자동차] -> 자동:[자동문, 자동차] -> 자동차:[자동차])

실제로 이런 내용이 단어 일 때는 문제가 없지만, 문장이나 여러단어가 된다면, 필터링 + 렌더링이 꽤나 무거운 작업이 된다.
그럼 이를 어떻게 해결하는게 좋을까? 아래의 코드를 보자.
```jsx
...
function SearchList({items}){
  ...
  const handleChange = (e) => {
    ...
    // 1. 입력 필드 값은 바로 반영(급한 작업)
    setQuery(value);

    // 2. 필터링은 startTransition안에 넣어서 "급하지 않은 작업"
    startTransition(() => {
...
이 코드의 핵심은 React의 Fiber엔진을 이용하여 
1. 사용자의 입력은 실시간으로 업데이트  
2. 리스트 필터링은 startTransition안에 넣어서 나중에 반영한다는 점이다. 

사용자 입장(UX)에서는
- 타이핑 반응은 빠르고
- 리스트는 약간 늦게 바뀌지만, 그 사이에 브라우저가 멈춘듯한 느낌은 적다는 것이다.
이것이 동시성 렌더링이 가져온 UX측면의 개선점인 것이다.

## 결론 : React의 학습커브가 왜 가파르게 느껴질까?
원점으로 돌아와서 학습커브가 복잡해지는 이유는 다음과 같다.
1. MVC -> Flux 패턴으로의 전환이해
2. 패턴이해에서 발생하는 Dispatcher과 Store이해
3. Function 컴포넌트의 개념의 이해를 위한 Class 컴포넌트 구조 이해와 Hooks 문법과 규칙이해
4. 랜더링과 useEffect, useState 사용타이밍과 이유 그리고 콜백 의존성 배열의 이해
5. Fiber의 쪼갠 구조의 엔진작동원리, 동시성랜더링, 이에 따른 useTransition같은 구조에 대한 맞는 기술의 최적화개념

결론적으로 DOM을 직접잡고 조작하던 JS코드에서 연쇄적인 업데이트(Cascading Update)를 해결하기 위해, 쪼개고 React로 프레이밍한 구조를 최적하기 위한 구조와 이유를 이해하면서 써야하니, 학습커브자체가 높을 수 밖에 없다는 것이다.

다만 장점으로는 이러한 사연과 개념을 이해하게 된다면, 리엑트로 인한 안정적인 코드의 유지보수와 코드의 책임영역별 피드백을 유연하게 줄 수 있다는 점에서 React를 공부할 가치가 크다고 개인적으로 생각한다.